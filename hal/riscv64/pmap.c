...
#define PTE(paddr, flags) (((paddr) >> 12) << 10 | (flags))
#define PTE_TO_ADDR(pte)  ((((pte) >> 10) << 12) & 0xfffffffffff000UL)
...
static int _pmap_enter(u64 *pdir2, addr_t pa, void *va, int attr, page_t *alloc, int tlbInval)
{
	addr_t addr;
	spinlock_ctx_t sc;
...
int pmap_enter(pmap_t *pmap, addr_t pa, void *va, int attr, page_t *alloc)
{
	...
	ret = _pmap_enter(pmap->pdir2, pa, va, attr, alloc, 1);
	if (ret == EOK) {
